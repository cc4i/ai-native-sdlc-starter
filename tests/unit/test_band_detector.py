...
class TestBandDetector(unittest.TestCase):
    ...
    def test_insufficient_baseline_data(self):
        ...
        result = self.detector.evaluate(current_value=15.0, baseline_points=baseline, metric_config=self.metric_config)
    ...
    def test_normal_variation_returns_log_tier(self):
        ...
        result = self.detector.evaluate(current_value=10.5, baseline_points=baseline, metric_config=self.metric_config)
    ...
    def test_below_min_absolute_change_stays_log_tier(self):
        ...
        baseline = [1.0] * 14
        result = self.detector.evaluate(current_value=3.0, baseline_points=baseline, metric_config=self.metric_config)
        self.assertEqual(result.tier, Tier.LOG)
        self.assertFalse(result.action_required)
    ...
    def test_moderate_deviation_returns_diagnose_tier(self):
        ...
        result = self.detector.evaluate(current_value=11.15, baseline_points=baseline, metric_config=config)
    ...
    def test_critical_spike_returns_act_tier(self):
        ...
        baseline = [9.0, 11.0, 10.0, 10.0, 9.5, 10.5, 9.8, 10.2, 9.7, 10.3, 10.1, 9.9, 10.0, 10.0]
        result = self.detector.evaluate(current_value=25.0, baseline_points=baseline, metric_config=self.metric_config)
